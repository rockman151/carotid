...
class Up_sampling(keras.layers.Layer):
    def __init__(self, size, name):
        super(Up_sampling, self).__init__(name=name)
        self.blocks = keras.Sequential()
        self.blocks.add(keras.layers.UpSampling2D(size= size))
    def __call__(self, inputs, **kwargs):
        output = self.blocks(inputs)
        return output
...
def Network(input_size):
    ...
    d4d = Conv_Bn_ReLU(nb_fils * 8, filter_size, (1, 1), name='layer17_conv_bn_relu')(d4c)
    ...
    d3d = Conv_Bn_ReLU(nb_fils * 4, filter_size, (1, 1), name='layer20_conv_bn_relu')(d3c)
    ...
    d2d = Conv_Bn_ReLU(nb_fils, filter_size, (1, 1), name='layer23_conv_bn_relu')(d2c)
    ...
    d1d = Conv_Bn_ReLU(nb_fils, filter_size, (1, 1), name='layer26_conv_bn_relu')(d1c)
    jump1 = Up_sampling(8,name = 'layers27_upsampling')(d4d)
    jump1c = Conv_Bn_ReLU(nb_fils, filter_size, (1, 1), name='layer28_conv_bn_relu')(jump1)
    jump2 = Up_sampling(4, name='layers29_upsampling')(d3d)
    jump2c = Conv_Bn_ReLU(nb_fils, filter_size, (1, 1), name='layer30_conv_bn_relu')(jump2)
    jump3 = Up_sampling(2, name='layers31_upsampling')(d2d)
    jump3c = Conv_Bn_ReLU(nb_fils, filter_size, (1, 1), name='layer32_conv_bn_relu')(jump3)
    output = Conv_Bn_ReLU(output_ch, 1, (1, 1), name='layer33_conv_bn')(tf.concat([d1d, jump1c,jump2c,jump3c], 3))
    model = tf.keras.Model(inputs=input_x, outputs=output)
    return model
def metrics_vw(y_true, y_pred):
...
for ite in range(10):
    if ite>0:
        model.load_weights('\Users\checkpoint_carotid\train_unet_best_carotid')
    ...
    model.save_weights('\Users\checkpoint_carotid\train_unet_best_carotid')
